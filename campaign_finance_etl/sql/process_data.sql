...
------------------------------------------------------------------------------------------------------------------------
alter table raw.ethics_report_2019_all_20201025
    add column id serial,
    add column ukey text;

update raw.ethics_report_2019_all_20201025 set ukey = '2019_' || id::text;
...
from raw.ethics_report_2019_all_20201025;
------------------------------------------------------------------------------------------------------------------------
...
insert into working.pacs (pac_name)
values
    ('Associated General Contractors of Georgia, Inc.PAC'),
    (''),
    (''),
    ('')
...
select
    extract(year from contribution_date) as contribution_year,
    count(*) as num_contributions,
    sum(cash_amount) as sum_cash_amount
from campaign_finance.fact_contributions
group by extract(year from contribution_date)
order by contribution_year;
...
from campaign_finance.fact_contributions
where extract(year from contribution_date) = 2020
group by extract(year from contribution_date), extract(month from contribution_date)
order by contribution_year, contribution_month;

select
    extract(year from date::date) as contribution_year,
    extract(month from date::date) as contribution_month,
    count(*) as num_contributions,
    sum(cash_amount) as sum_cash_amount
